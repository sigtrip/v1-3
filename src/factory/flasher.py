...
SMART_FIRMWARES = {
    "gateway": {"kind": "auto", "description": "Авто-заготовка под распознанный чип"},
    "air_tag": {"kind": "auto", "description": "Авто-заготовка для air_tag"},
...
class AirFlasher:
    def list_available_firmwares(self) -> list[str]:
        ...
        for fw_name, meta in SMART_FIRMWARES.items():
            if meta.get("kind") == "auto":
                ...
                continue
            fw_path = meta.get("path", "")
            if fw_path and os.path.exists(fw_path):
                names.append(fw_name)
            else:
                names.append(f"{fw_name} (ожидается {fw_path})")
        return names
    ...
    def _resolve_firmware_path(self, chip: str, target_name: str) -> tuple[str, str | None]:
        target_key = (target_name or "gateway").strip().lower()
        meta = SMART_FIRMWARES.get(target_key)
        if meta and meta.get("kind") == "bin":
            fw_path = str(meta.get("path", "") or "")
            if not os.path.exists(fw_path):
                return "", f"⚠️ Бинарник '{target_key}' не найден: {fw_path}"
            return fw_path, None
        return self._build_firmware_stub(chip, target_name=target_key), None
    ...
    def smart_flash(self, port: str = None, target_name: str = "gateway") -> str:
        ...
        fw, resolve_note = self._resolve_firmware_path(selected["chip"], target_name=target_name)
        if not fw:
            ...
            return (
                ...
                f"  Чип: {selected['chip']}\n"
                f"  Ошибка: {resolve_note or 'Не удалось определить прошивку'}\n"
