...
class ArgosMobileUI(App):
    ...
    def build(self):
        ...
        self.orb = QuantumOrb(
            text="[color=00ffff]●[/color]", font_size="90sp", markup=True, size_hint_x=None, width=110
        )
        top.add_widget(self.orb)

        info = BoxLayout(orientation="vertical")
        self.state_lbl = Label(text="Состояние: Analytic", halign="left", font_size="13sp", color=(0, 1, 0.53, 1))
        self.health_lbl = Label(text="Shield: Active", halign="left", font_size="11sp", color=(0.7, 0.9, 1, 1))
        ...
        listen_btn = Button(text="🎙 Слушай меня", font_size="13sp", background_color=(0.1, 0.4, 0.1, 1))
        ...
        send_btn = Button(text="▶", size_hint_x=None, width=50, font_size="18sp", background_color=(0, 0.4, 1, 1))
        send_btn.bind(on_press=lambda _: self._send(self.entry.text))
        inp.add_widget(send_btn)

        mic_btn = Button(text="🎤", size_hint_x=None, width=50, font_size="16sp", background_color=(0.1, 0.4, 0.1, 1))
