...
SYSTEM_PROFILES = {
    ...
    "incubator": {
        ...
        "rules": [
            ...
            {"if": "humidity < 50", "then": "humidifier:on", "msg": "Влажность низкая — увлажнение"},
    ...
    "terrarium": {
        ...
        "rules": [
            {"if": "temp_hot < 30", "then": "lamp_heat:on", "msg": "Холодно — нагревательная лампа"},
...
class SmartSystem:
    ...
    def update_sensor(self, sensor: str, value):
        ...
        bus.publish(EventType.SENSOR_UPDATE, {"system": self.id, "sensor": sensor, "value": value}, "smart")
        self._check_rules()
    ...
    def set_actuator(self, actuator: str, state: str):
        ...
        if old != state:
            ...
            bus.publish(EventType.DEVICE_CMD, {"system": self.id, "actuator": actuator, "state": state}, "smart")
        ...
        for rule in self.rules + self.custom_rules:
            try:
                ...
                if _safe_eval_condition(expr, {"time": time.strftime("%H:%M")}):
                    ...
                    bus.publish(EventType.SMART_RULE_FIRE, {"rule": cond, "action": action, "msg": msg}, "smart")
