...
class ArgosIconGenerator:
    ...
    def _draw_icon(self, size: int) -> "Image":
        ...
        draw.ellipse(
            [cx - r + ring_w, cy - r + ring_w,
             cx + r - ring_w, cy + r - ring_w],
            outline=(0, 220, 220, 255), width=ring_w
        )
        ...
        for i in range(1, grid_lines):
            ...
            alpha = 25
            draw.line([(x, cy - r), (x, cy + r)],
                      fill=(0, 100, 120, alpha), width=1)
            draw.line([(cx - r, y), (cx + r, y)],
                      fill=(0, 100, 120, alpha), width=1)
        ...
        draw.line(eyelid_pts_top + [eyelid_pts_top[0]],
                  fill=(0, 200, 220, 200), width=max(1, size//48))
        draw.line(eyelid_pts_bot + [eyelid_pts_bot[0]],
                  fill=(0, 200, 220, 200), width=max(1, size//48))
        ...
        for offset in range(4, 0, -1):
            alpha = 60 * offset // 4
            draw.ellipse(
                [cx - iris_r - offset, cy - iris_r - offset,
                 cx + iris_r + offset, cy + iris_r + offset],
                outline=(0, 255, 255, alpha), width=1
            )
        draw.ellipse(
        ...
        draw.ellipse(
            [cx - pupil_r + shine_r, cy - pupil_r + shine_r,
             cx - pupil_r + shine_r * 3, cy - pupil_r + shine_r * 3],
    ...
    # ── СОХРАНЕНИЕ ФОРМАТОВ ───────────────────────────────
    def generate_png(self, size: int = 512,
                     path: str = None) -> str:
        if not PIL_OK:
    ...
    def generate_icns(self, path: str = None) -> str:
        ...
        icns_sizes = {
            b"is32": 16, b"il32": 32, b"ih32": 48,
            b"it32": 128, b"ic08": 256, b"ic09": 512,
        }
        buf = io.BytesIO()
