...
class NFCManager:
    ...
    def _init_desktop_nfc(self):
        ...
        try:
            ...
            if available_readers:
                ...
                log.info("NFC: Desktop-адаптер (pyscard) найден: %s", [str(r) for r in available_readers])
                return
        except Exception as e:
    ...
    def _save_tags(self):
        ...
        try:
            ...
            with open(NFC_TAGS_FILE, "w", encoding="utf-8") as f:
                json.dump([t.to_dict() for t in self._tags.values()], f, ensure_ascii=False, indent=2)
        except Exception as e:
            log.error("NFC: ошибка сохранения меток: %s", e)

    @trace("nfc.register_tag")
    def register_tag(
        self, uid: str, name: str, action: TagAction, action_data: Dict[str, Any], location: str = ""
    ) -> NFCTag:
        """
    ...
    def _read_tag(self) -> Optional[str]:
        ...
        # nfcpy
        try:
            tag = self._nfc_adapter.connect(rdwr={"on-connect": lambda t: False}, terminate=lambda: not self._scanning)
            if tag:
    ...
    def _handle_tag(self, uid: str):
        ...
        if tag:
            ...
            if bus:
                bus.emit(
                    Events.NFC_TAG_DETECTED,
                    {"uid": uid, "name": tag.name, "action": tag.action, "action_data": tag.action_data},
                    "nfc_manager",
                )
    ...
    def scan_single(self, timeout: float = 5.0) -> Optional[Dict[str, Any]]:
        ...
        while time.time() - start < timeout:
            ...
            if uid:
                ...
                tag_info = {"uid": uid, "type": self._detect_tag_type(uid), "registered": uid in self._tags}
