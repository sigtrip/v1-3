...
class USBDiagnostics:
    ...
    def _load_authorized(self):
        ...
        if os.path.exists(USB_DEVICES_FILE):
            try:
                ...
                log.info("USB: загружено %d авторизованных устройств", 
    ...
    def _save_authorized(self):
        ...
        try:
            ...
            with open(USB_DEVICES_FILE, "w", encoding="utf-8") as f:
                json.dump([d.to_dict() for d in self._authorized.values()], f,
        ...
        try:
            ...
            for port in serial.tools.list_ports.comports():
                dev_info = {
                    ...
                    "authorized": self._is_authorized(port.vid, port.pid, 
                                                      port.serial_number)
                }
    ...
    def authorize_device(self, vid_pid: str, name: str, 
                         device_type: DeviceType,
                         serial: str = "",
                         baudrate: int = 115200,
                         notes: str = "") -> AuthorizedDevice:
        """
    ...
    @trace("usb.connect")
    def connect(self, port_or_vid_pid: str, 
                baudrate: int = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            log.info("USB: подключено к '%s' на %s @ %d", 
                    device.name, port, baudrate)
    ...
    def read_continuous(self, port: str, callback: Callable[[str], None],
    ...
    def upload_firmware(self, port: str, firmware_path: str,
                        device_type: DeviceType = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if device_type in [DeviceType.ARDUINO_UNO, DeviceType.ARDUINO_NANO,
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "scan":
            ...
            for d in devices:
                ...
                print(f"[{auth}] {d['port']}: {d['description']} "
                      f"({d['vid']}:{d['pid']}) - {d['device_type']}")
