...
- Для просмотра реальной матрицы поддержки на ноде: `iot возможности`.
- В матрице отражены поддерживаемые Zigbee-хабы и USB/Ethernet-координаторы (Aqara/Xiaomi/Яндекс/Tuya/Hubitat, Sonoff/SkyConnect/ConBee/SMLIGHT и др.).
- Конфигурация через `.env` загружается bootstrap-механизмом (из текущей директории запуска с fallback в корень репозитория).
